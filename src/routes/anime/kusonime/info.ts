    ...
    async (request: FastifyRequest<{ Params: { endpoint: string } }>, reply: FastifyReply) => {
      ...
      try {
        ...
        const animeInfo = await page.evaluate(() => {
          ...
          const additionalInfoTags = Array.from(document.querySelectorAll('.info p')).map(
            (p) => p.textContent
          );
          ...
          const excludeTags = ['b'];
          const descriptionElements = Array.from(lexotElement.querySelectorAll('p'))
            .filter((p) => !excludeClasses.some((cls) => p.classList.contains(cls)))
            .filter((p) => !excludeTags.some((tag) => p.getElementsByTagName(tag).length > 0));

          return {
            id: window.location.href?.split('/')[4],
            title: document.querySelector('h1.jdlz')?.textContent,
            url: window.location.href,
            thumbnail: document.querySelector('img.attachment-thumb-large')?.getAttribute('src'),
            releaseDate,
            description: descriptionElements.map((p) => p.textContent?.trim()).join('\n'),
            genres,
            type,
            status,
            alternativeTitle,
